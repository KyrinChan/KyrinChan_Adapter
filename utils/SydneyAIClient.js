...
export default class SydneyAIClient {
  ...
  async sendMessage (
    ...
    const moodTip = 'Your response should be divided into two parts, namely, the text and your mood. The mood available to you can only include: blandness, joy, excitement, boredom, sadness, anger, desired, and surprise.At the same time, it is also necessary to rate the favorability of the other party, similar to the GalGame game, with a value range from 0 to 100,This rating should be easy to decrease, but not easy to increase.All content should be replied in this format {"text": "", "mood": "", "favor": ""}.All content except mood should be placed in text, It is important to ensure that the content you reply to can be parsed by json.'
      ...
      `
        context += chats
          .map(chat => {
            ...
            return `【${sender.card || sender.nickname}】（识别代码：${sender.user_id}，${roleMap[sender.role] || '一般成员'}，${sender.area ? '来自' + sender.area + '，' : ''} ${sender.age}岁， 群头衔：${sender.title}， 性别：${sender.sex}，时间：${formatDate(new Date(chat.time * 1000))}） 说：${chat.raw_message}`
          })
          .join('\n')
      }
    }
