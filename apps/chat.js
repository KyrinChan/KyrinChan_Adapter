      ...
      if (blockWord) {
        ...
        if (Config.Gen6Standalone) {
          ...
          if (Config.Gen6Impressions) {
            ...
            if (fs.existsSync(filepath)) {
              let msg = '请根据这段对话的内容以凯琳酱的设定来看一下这份印象档案，它包含在一个json格式的字符串里 "' + userjson + '" 其中的数据具体为"' + Config.impressionDefinition + '"，要根据凯琳酱的人设和上下文判断有没有需要修改或追加的内容，回复应当只包含该文档的字符串形式json，而且需要严格按照json语法完成，以确保能够解析。'
