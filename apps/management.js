    ...
    if (e.msg.indexOf('本群') > -1) {
      if (e.isGroup) {
        ...
        if (await redis.get(`CHATGPT:SHUT_UP:${scope}`)) {
          ...
          await this.reply(`好的，凯琳酱在这里就不说话了喵~：倒计时${formatDuration(time)}`)
        } else {
          await redis.set(`CHATGPT:SHUT_UP:${scope}`, '1', { EX: time })
          await e.reply(`好的，凯琳酱在这里就不说话了喵~：倒计时${formatDuration(time)}`)
        }
      } else {
        await e.reply('喵，这里好像不是群哦')
        return false
      }
    } else if (match) {
      ...
      if (e.bot.getGroupList().get(groupId)) {
        if (await redis.get(`CHATGPT:SHUT_UP:${groupId}`)) {
          ...
          await this.reply(`好的，凯琳酱在${groupId}就不说话了喵~${formatDuration(time)}`)
        } else {
          await redis.set(`CHATGPT:SHUT_UP:${groupId}`, '1', { EX: time })
          await this.reply(`好的，凯琳酱在${groupId}就不说话了喵~${formatDuration(time)}`)
        }
      ...
      if (await redis.get('CHATGPT:SHUT_UP:ALL')) {
        ...
        await this.reply(`好的，再加上${formatDuration(time)}咯~`)
      } else {
        await redis.set('CHATGPT:SHUT_UP:ALL', '1', { EX: time })
        await this.reply(`好的，没问题哒${formatDuration(time)}`)
      }
    }
  }
  ...
  async openMouth (e) {
    ...
    if (e.msg.indexOf('本群') > -1) {
      ...
      if (e.bot.getGroupList().get(groupId)) {
        if (await redis.get(`CHATGPT:SHUT_UP:${groupId}`)) {
          await redis.del(`CHATGPT:SHUT_UP:${groupId}`)
          await this.reply(`好的捏，在${groupId}可以说话了捏！`)
        } else {
